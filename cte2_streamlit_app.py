...
# ---------- 语音按钮（内联onclick，直接修改文本框）----------
def voice_input_script():
    html = """
    <div style="margin:8px 0; display:flex; align-items:center; gap:10px;">
        <button id="voiceBtnMain" style="
            padding:10px 20px; font-size:16px; border:none; border-radius:6px;
            background-color:#4CAF50; color:white; cursor:pointer;
        " onclick="toggleVoice()">🎤 开始录音</button>
        <span id="voiceStatusMain" style="color:#666;">点击后说话</span>
    </div>
    <script>
    var recognitionMain = null;
    var isRecordingMain = false;
    function toggleVoice() {
        var btn = document.getElementById('voiceBtnMain');
        var status = document.getElementById('voiceStatusMain');
        if (isRecordingMain) {
            if (recognitionMain) { recognitionMain.stop(); recognitionMain = null; }
        ...
        recognitionMain.onerror = function(event) {
            status.innerText = '❌ 错误: ' + event.error;
            btn.innerText = '🎤 开始录音';
            btn.style.backgroundColor = '#4CAF50';
            isRecordingMain = false;
        };
        recognitionMain.onend = function() {
            if (isRecordingMain) {
                isRecordingMain = false;
                btn.innerText = '🎤 开始录音';
                btn.style.backgroundColor = '#4CAF50';
